...
| Float wallet auth | Per-request ed25519 (`x-actor` + `x-owner-ts` + `x-owner-sig`). Connected wallet then signs the Solana tx (`mode: "chain"`); HUD updates only after `POST /chain/confirm`. Loopback unsigned `owner_demo` remains. Mainnet gated by `MARKOV_MAINNET=1` | API tests + `buildSubscribe` unsigned tx 2026-08-28. Live `bun scripts/float-chain-subscribe.ts` recorded in SESSION_LOG when it lands |
